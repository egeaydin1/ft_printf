...
/*   Updated: 2025/06/27 21:03:29 by egeaydin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	printer(char type, va_list list)
{
	...
	if (type == 'x')
		size += ft_putnbr_base((unsigned int)va_arg(list, int), 16, 'x',0);
	if (type == 'X')
		size += ft_putnbr_base((unsigned int)va_arg(list, int), 16, 'X',0);
	if (type == 'u')
		size += ft_putnbr_base((unsigned int)va_arg(list, int), 10, 'u',0);
	if (type == 'p')
		size += ft_putnbr_base((unsigned long int)va_arg(list, void *), 16,
				'p',1);
	return (size);
}
